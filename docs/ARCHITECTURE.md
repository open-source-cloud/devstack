...
| `internal/config` | Workspace discovery (walk-up), goccy parse with positions, JSON-Schema + validator/v10 + custom cross-ref/cycle resolver, profile/env overlay merge, immutable loaded model, embedded schemas, the `${env/self/ref/profile}` grammar. *(v2 additive schema: the `resources`/`platform` block — see [spec 18](specs/18-resource-limits-and-multi-arch.md); the `templates:`/`verify:` lockfile blocks — see [spec 19](specs/19-template-registry.md); the `sanitize:` profile — see [spec 15](specs/15-db-snapshot-restore.md); `telemetry.*` lives in a separate user file — see [spec 20](specs/20-telemetry.md).)* |
| `internal/template` | Text-template engine + ported filters; `renderText`/`renderYAML` modes; `extends`-chain resolution; `TemplateSource` interface (embedded for v1; git/OCI later); golden-test harness. *(v2: the OCI/git registry workflow + digest pinning is layered on by `internal/registry` — see [spec 19](specs/19-template-registry.md).)* |
...
| `internal/generate` | Compose-model build + `compose-go` validate/normalize, `${ref}` resolution against the workspace graph, secret-key emission, `writeIfChanged` + atomic rename, SHA-256 rebuild-hash ledger. *(v2: emits `deploy.resources`/`mem_limit`/`platform:` from the `resources` block — see [spec 18](specs/18-resource-limits-and-multi-arch.md); IDE artifacts ride the same deterministic pipeline via `internal/ide` — see [spec 17](specs/17-devcontainer-ide-integration.md).)* |
...
| `internal/db` *(v2)* | **Post-1.0.** Per-project (tenant-scoped) data lifecycle on the shared engines — `db snapshot`/`restore`/`reset`/`list`/`pull`, content-addressed dump store, engine dumpers (`pg_dump`/`redis`/`mc`) behind a `Dumper` interface; reuses `internal/provision` + the flock. See [spec 15](specs/15-db-snapshot-restore.md). |
| `internal/dashboard` *(v2)* | **Post-1.0.** Read-only bubbletea cockpit + multiplexed `logs`: fans in label-filtered `ContainerList`/`ContainerLogs`/`ContainerStats`/`Events` + the ledger snapshot; **lock-free, never mutates**. See [spec 16](specs/16-logs-and-dashboard.md). |
| `internal/ide` *(v2)* | **Post-1.0.** Editor-artifact generation sink (`ide gen`): per-repo `.devcontainer/devcontainer.json`, multi-root `.code-workspace`, debugger `launch.json`, `$schema` modeline; rides the deterministic pipeline + managed-block merge; debug-port allocation through the workspace flock. See [spec 17](specs/17-devcontainer-ide-integration.md). |
| `internal/telemetry` *(later)* | **Post-1.0.** Strictly opt-in (default OFF), allowlist-only, best-effort pure-Go OTLP/HTTP export of command/outcome/os/arch; reuses the spec-14 notifier suppression discipline; deliberately **off** the lock/state spine. See [spec 20](specs/20-telemetry.md). |
| `internal/registry` *(v2)* | **Post-1.0.** Versioned template registry around the `TemplateSource` seam: OCI (`oras-go/v2`) + git pull, digest-pinned `templates:` lockfile, `push`/`add`/`update`/`diff`/`verify`, optional cosign verification; cache-write under the flock. Extends `internal/template`. See [spec 19](specs/19-template-registry.md). |
| `internal/docker` `Backend` seam *(later, exploratory)* | **Post-1.0.** Remote/cloud shared-services backend abstraction: `Local`/`Remote` backends selected by Docker context, generalizes the local flock to a `DistLock` (Postgres advisory lock / coordinator), per-user tenant isolation, `ViaProxy` reachability. Extends `internal/workspace` + `internal/docker`. See [spec 21](specs/21-remote-shared-backend.md). |
