...
func addStubCommands(root *cobra.Command, _ *GlobalOpts) {
	root.AddCommand(
		...
		stub("template", "Author and validate service templates", "M1",
			stub("init", "Scaffold a new template", "M1"),
			stub("lint", "Lint a template", "M1"),
			stub("test", "Run a template's golden tests", "M1"),
		),
